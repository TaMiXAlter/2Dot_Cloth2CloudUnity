void AnimatedOrganic_float(bool autoColor, float scale, float scaleMultiplicationStep,
                           ...
                           float Iterations, float2 uv, float time, float4 MainColor,float4 CameraColor,
                           out float output,out float4 color)
{
    ...
    if (autoColor)
    {
        ...
        float t = 0.5 + 0.5 * sin(time + output * 3);
        
        float3 mixedColor= lerp(MainColor, white, t);

        if(all(CameraColor.rgb != float3(1.0,1.0,1.0)) && output > 0.8){
            mixedColor = lerp(CameraColor.rgb, white, t);
        }
